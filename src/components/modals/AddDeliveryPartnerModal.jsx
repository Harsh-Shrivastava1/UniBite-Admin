...
const AddDeliveryPartnerModal = ({ isOpen, onClose }) => {
    const { addDeliveryPartner } = useAdmin();
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-card rounded-xl border border-border w-full max-w-md p-6 shadow-2xl scale-100 animate-in zoom-in-95 duration-200">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-bold text-foreground">Add Delivery Partner</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-muted-foreground mb-1">Partner Name</label>
                        <input
                            type="text"
                            required
                            value={formData.name}
                    ...
                    <div className="flex space-x-3 mt-6">
                        ...
                        >
                            {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Add Partner'}
                        </button>
                    </div>
                </form>
            </div>
